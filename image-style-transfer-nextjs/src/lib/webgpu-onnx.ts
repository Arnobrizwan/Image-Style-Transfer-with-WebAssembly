export interface WebGPUONNXRunner {
...
export class WebGPUONNXRunnerImpl implements WebGPUONNXRunner {
  ...
  async processImage(imageData: ImageData, styleStrength: number): Promise<ImageData> {
    ...
    // Create compute pipeline for neural style transfer
    const shaderModule = this.device.createShaderModule({
      code: `
        ...
        fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
          ...
          // this would be the actual neural network computation
          
          // Van Gogh style effect
          ...
          output[index + 3] = a; // Preserve alpha
        }
      `
    });
...
export class CPUFallbackONNXRunner implements WebGPUONNXRunner {
  async initialize(): Promise<boolean> {
    console.log('Using CPU fallback ONNX runner');
    return true;
  }
...
export async function createONNXRunner(): Promise<WebGPUONNXRunner> {
  const webgpuRunner = new WebGPUONNXRunnerImpl();
  
  if (await webgpuRunner.isSupported()) {
    try {
      const initialized = await webgpuRunner.initialize();
      if (initialized) {
        console.log('WebGPU ONNX runner created successfully');
        return webgpuRunner;
      }
    } catch (error) {
  ...
  console.log('Using CPU fallback ONNX runner');
  return new CPUFallbackONNXRunner();
}
